...
| --- | --- |
| `tools.packages`, `image.name`, or Coop's embedded image inputs | Run `coop rebuild`; the next `coop up` or entry recreates the container |
...
4. operating-system fallback paths from the image.

The core wins collisions with configured packages. Use a project `.flox` when
the repository deliberately needs its own runtime version to win command
lookup or wants the same environment outside Coop.
...
Use `[tools].packages` for additive Linux commands that should be present in the
Coop image. Use a project `.flox` when the project needs pinned runtime versions
or the same environment inside and outside Coop. Flox is optional; Coop detects
and activates the nearest `.flox` between the current directory and project
root.
